...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        {
            return Ok(self.chunks.swap_remove(index));
        }
        Err(anyhow!("Chunk with chunk type could not be found"))
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        let (header, mut chunks_bytes) = bytes.split_at(Png::STANDARD_HEADER.len());
        if header != Png::STANDARD_HEADER {
            return Err(anyhow!("Standard header was not valid"));
        }
        let mut chunks: Vec<Chunk> = Vec::new();
        loop {
            let chunk_length = &chunks_bytes[..Chunk::DATA_LENGTH_BYTES];
            let chunk_length: usize = u32::from_be_bytes(chunk_length.try_into()?) as usize;

            let total_chunk_length = Chunk::DATA_LENGTH_BYTES
                + Chunk::CHUNK_TYPE_BYTES
                + chunk_length
                + Chunk::CRC_BYTES;
            let (curr_chunk_bytes, chunks_bytes_temp) = chunks_bytes.split_at(total_chunk_length);
            let curr_chunk = Chunk::try_from(curr_chunk_bytes)?;
            chunks.push(curr_chunk);
            match chunks_bytes_temp.len() {
                0 => break,
                length if length < Chunk::METADATA_BYTES => return Err(anyhow!("Error in chunk")),
                _ => chunks_bytes = chunks_bytes_temp,
            }
        }
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "header:{:?}, chunks:{:?}", self.header(), self.chunks)
    }
}
